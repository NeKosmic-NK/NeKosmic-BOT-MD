...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        if (!m.fromMem && m.text.match(/(NeKosmic|nekosmic|NeKosmic Bot|nekosmic-bot)/gi)) {
        let emot = pickRandom(["🎃", "❤", "😘", "😍", "💕", "😎", "🙌", "⭐", "👻", "🔥"])
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*𝚂𝙸𝙽 𝙳𝙴𝚂𝙲𝚁𝙸𝙿𝙲𝙸𝙾𝙽*') :
                            ...
                            this.sendHydrated(id, text, groupMetadata.subject, apii.data, 'https://www.instagram.com/invites/contact/?i=tol351ueuxke&utm_content=ohvhscn', '👑𝙸𝙽𝚂𝚃𝙰𝙶𝚁𝙰𝙼👑', null, null, [
